...
function comparePlatform(platform, bitness = '') {
  ...
  if (
    platform.indexOf('x64') !== -1 ||
    (platform.indexOf('x86') !== -1 && bitness === '64')
  ) {
    return 'x64';
  }
...
class OsAbstractParser extends ParserAbstract {
  ...
  parseFromUserAgent(userAgent) {
    ...
    for (let i = 0, l = this.collection.length; i < l; i++) {
      ...
      if (match !== null) {
        let { name, short } = this.getOsDataByName(
          this.buildByMatch(item.name, match),
        );

        let version =
          item.version !== void 0 ? this.buildVersion(item.version, match) : '';
  ...
  parsePlatform(userAgent) {
    ...
    if (
      this.getBaseRegExp('.+32bit|.+win32|(?:i[0-9]|x)86|i86pc').test(userAgent)
    ) {
      return 'x86';
    }
