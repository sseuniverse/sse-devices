...
class DeviceDetector {
  ...
  init() {
    ...
    this.addParseClient(
      CLIENT_PARSER_LIST.MEDIA_PLAYER,
  ...
  hasBrand(brand) {
    return (
      this.getParseDevice(DEVICE_PARSER_LIST.MOBILE).getCollectionBrands()[
        brand
      ] !== void 0
    );
  }
  ...
  getAvailableBrowsers() {
    return this.getParseClient(
      CLIENT_PARSER_LIST.BROWSER,
    ).getAvailableBrowsers();
  }
  ...
  prepareUserAgent(userAgent) {
    if (
      userAgent &&
      this.maxUserAgentSize &&
      this.maxUserAgentSize < userAgent.length
    ) {
   ...
   * @return {DeviceType}
   */
  parseDeviceType(userAgent, osData, clientData, deviceData, clientHints) {
  ...
  restoreUserAgentFromClientHints(userAgent, clientHints) {
    ...
    if (
      deviceModel &&
      /Android 10[.\d]*; K(?: Build\/|[;)])/i.test(userAgent)
    ) {
  ...
  parseDevice(userAgent, clientHints) {
    ...
    if (this.deviceInfo || this.deviceTrusted) {
      result.info = this.getParseInfoDevice().info(
        result.brand,
        result.code ? result.code : result.model,
      );
    }
  ...
  prepareDetectResult(userAgent, osData, clientData, deviceData, clientHints) {
    /**
    ...
    if (this.deviceTrusted) {
      deviceData.trusted = DeviceTrusted.check(
        osData,
        clientData,
        deviceData,
        clientHints,
